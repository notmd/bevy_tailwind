...
pub fn parse_padding(ctx: &mut ParseCtx, class: &str) -> ParseResult {
    ...
    if class.starts_with("pt-") {
        ...
        insert_node_ui_rect_props!(ctx, NodeProp::Padding, val, 2, [top]);
    }

    if class.starts_with("pr-") {
        let val = parse_padding_val(class, neg)?;
        insert_node_ui_rect_props!(ctx, NodeProp::Padding, val, 2, [right]);
    }

    if class.starts_with("pb-") {
        let val = parse_padding_val(class, neg)?;
        insert_node_ui_rect_props!(ctx, NodeProp::Padding, val, 2, [bottom]);
    }

    if class.starts_with("pl-") {
        let val = parse_padding_val(class, neg)?;
        insert_node_ui_rect_props!(ctx, NodeProp::Padding, val, 2, [left]);
    }

    if class.starts_with("px-") {
        let val = parse_padding_val(class, neg)?;
        insert_node_ui_rect_props!(ctx, NodeProp::Padding, val, 1, [left, right]);
    }

    if class.starts_with("py-") {
        let val = parse_padding_val(class, neg)?;
        insert_node_ui_rect_props!(ctx, NodeProp::Padding, val, 1, [top, bottom]);
    }

    if class.starts_with("p-") {
        let val = parse_padding_val(class, neg)?;
        insert_node_ui_rect_props!(ctx, NodeProp::Padding, val, 0, [top, right, bottom, left]);
    }
...
pub fn parse_margin(ctx: &mut ParseCtx, class: &str) -> ParseResult {
    if class.starts_with("mt-") {
        ...
        insert_node_ui_rect_props!(ctx, NodeProp::Margin, val, 2, [top]);
    }

    if class.starts_with("mr-") {
        let val = parse_margin_val(class)?;
        insert_node_ui_rect_props!(ctx, NodeProp::Margin, val, 2, [right]);
    }

    if class.starts_with("mb-") {
        let val = parse_margin_val(class)?;
        insert_node_ui_rect_props!(ctx, NodeProp::Margin, val, 2, [bottom]);
    }

    if class.starts_with("ml-") {
        let val = parse_margin_val(class)?;
        insert_node_ui_rect_props!(ctx, NodeProp::Margin, val, 2, [left]);
    }

    if class.starts_with("mx-") {
        let val = parse_margin_val(class)?;
        insert_node_ui_rect_props!(ctx, NodeProp::Margin, val, 1, [left, right]);
    }

    if class.starts_with("my-") {
        let val = parse_margin_val(class)?;
        insert_node_ui_rect_props!(ctx, NodeProp::Margin, val, 1, [top, bottom]);
    }

    if class.starts_with("m-") {
        let val = parse_margin_val(class)?;
        insert_node_ui_rect_props!(ctx, NodeProp::Margin, val, 0, [top, right, bottom, left]);
    }
